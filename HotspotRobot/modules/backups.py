...
import HotspotRobot.modules.sql.blacklist_sql as blacklistsql
import HotspotRobot.modules.sql.locks_sql as locksql
import HotspotRobot.modules.sql.notes_sql as sql
import HotspotRobot.modules.sql.rules_sql as rulessql
from HotspotRobot import JOIN_LOGGER, LOGGER, OWNER_ID, SUPPORT_CHAT, dispatcher
from HotspotRobot.__main__ import DATA_IMPORT
from HotspotRobot.modules.connection import connected
from HotspotRobot.modules.helper_funcs.alternate import typing_action
from HotspotRobot.modules.helper_funcs.chat_status import user_admin
from HotspotRobot.modules.sql import disable_sql as disabledsql
...
def export_data(update, context):
    ...
    with open("HotspotRobot{}.backup".format(chat_id), "w") as f:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("HotspotRobot{}.backup".format(chat_id), "rb"),
        caption="» *ꜱᴜᴄᴄᴇꜱꜱꜰᴜʟʟʏ ᴇxᴘᴏʀᴛᴇᴅ ʙᴀᴄᴋᴜᴘ:*\n - ᴄʜᴀᴛ: `{}`\n - ᴄʜᴀᴛ ɪᴅ: `{}`\n - ᴏɴ: `{}`\n\n⚠ 𝗡𝗼𝘁𝗲: ᴛʜɪꜱ `HotspotRobot-Backup` ᴡᴀꜱ ꜱᴘᴇᴄɪᴀʟʟʏ ᴍᴀᴅᴇ ꜰᴏʀ ɴᴏᴛᴇꜱ 📚.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("HotspotRobot{}.backup".format(chat_id))  # Cleaning file
