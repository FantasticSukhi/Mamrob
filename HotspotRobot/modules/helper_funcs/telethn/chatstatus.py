...
from HotspotRobot import DRAGONS, telethn
from HotspotRobot.modules.helper_funcs.telethn import IMMUNE_USERS
...
async def hotspot_is_admin(chat_id: int):
    status = False
    hotspot = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if hotspot.id == user.id:
            status = True
            break
    return status
